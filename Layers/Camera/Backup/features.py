...
class PointCloudProcessor:
    def __init__(self, wCoordinates, eps=0.5, min_samples=50):
        ...
        self.points = np.array([[p[0], p[1], p[2]] for p in wCoordinates])  # Extract XYZ only
        self.colors = np.array([[p[3], p[4], p[5]] for p in wCoordinates]) / 255.0  # Extract RGB and normalize
    ...
    def visualize_point_cloud_with_contour(self, contour, centroid):
        ...
        centroid_color = self.colors[closest_idx]
        ...
        # Update the visualizer to ensure all geometries are rendered
        vis.update_geometry(o3d_pc)
        vis.update_geometry(line_set)
        vis.update_geometry(centroid_pc)
        vis.update_geometry(z_line_set)
        vis.update_renderer()
