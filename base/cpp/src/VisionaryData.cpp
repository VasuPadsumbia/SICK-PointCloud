...
//static constexpr float kBadPoint = std::numeric_limits<float>::quiet_NaN();
static constexpr float kBadPoint = 0.0f;
...
std::size_t VisionaryData::getItemLength(const std::string& dataType)
{
...
void VisionaryData::preCalcCamInfo(const ImageType& imgType)
{
  ...
  if (imgType == UNKNOWN)
  {
    throw std::invalid_argument("Unknown image type for the point cloud transformation");
  }
                                       ...
                                       std::vector<PointXYZ>&       pointCloud)
{
  ...
  // ++itMap, ++itUndistorted)
  {
    ...
    if (*itMap == 0 || *itMap == uint16_t(0xFFFF))
    {
      ...
      point.y = kBadPoint;
      point.z = kBadPoint;
      //std::cout << "Invalid map value: " << *itMap << " at index " << i << std::endl;
    }
    else
    {
...
const std::vector<std::uint32_t>& VisionaryData::getRGBAMap() const
{
  static const std::vector<std::uint32_t> empty;
  return empty;
}

const std::vector<std::uint16_t>& VisionaryData::getIntensityMap() const
{
